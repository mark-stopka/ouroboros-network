...
pureSnocket remoteAddresses =
  ...
  where
    ...
    accept :: FD m -> Accept m SomeException Addr (FD m)
...
prop_connectionManager myAddress' (Connections conns) =
  ...
  where
        ...
        withConnectionManager
          ...
          $ \connectionManager -> do
            ...
            let go :: [Async (IOSim s) (Connected Addr Handle Void)]
                   -> Accept (IOSim s) SomeException Addr (FD (IOSim s))
                   -> [ConnDescription PeerAddr]
            ...
            threads <- go [] (accept connectionSnocket fd) conns
    ...
    connectionSnocket :: forall s. Snocket (IOSim s) (FD (IOSim s)) Addr
    connectionSnocket = pureSnocket inboundAddresses
...
cmtPeerAddr (TrConnectionFinished connId _)     = Just (remoteAddress connId)
